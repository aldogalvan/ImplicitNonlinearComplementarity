...
//        VectorXd lambda(lambda_normal.size() + lambda_friction.size()); lambda.setZero();
        ...
        for (int bidx = 0; bidx < num_objects; bidx++)
        {
            ...
            M_tilde.block<6, 6>(6 * bidx, 6 * bidx) = m_objects[bidx]->generalized_mass();
            q.block<3,1>(6*bidx,0) = m_objects[bidx]->x;
            q(6*bidx+3) = m_objects[bidx]->q.w();
            q(6*bidx+4) = m_objects[bidx]->q.x();
            q(6*bidx+5) = m_objects[bidx]->q.y();
            q(6*bidx+6) = m_objects[bidx]->q.z();
        ...
        for (int n = 0; n < newton_iterations; n++)
        {
            ...
            for (int cidx = 0; cidx < num_contacts; cidx++) {
                ...
                // object indices
                auto &objectA = m_objects[contact->objectIdxA];
                auto &objectB = m_objects[contact->objectIdxB];

                // set the contact index
                int body_idxA = contact->objectIdxA;
                int body_idxB = contact->objectIdxB;
                ...
                Vector3d contact_wrt_objectA = contact->contact_wrt_objectA;
                Vector3d contact_wrt_objectB = contact->contact_wrt_objectB;
                ...
                // get the contact pts
                Vector3d contact_ptA = rot_A * contact_wrt_objectA + pos_A;
                cout << "DEBUG: contact_wrt_objectA = " << contact_wrt_objectA.transpose() << endl;
                cout << "DEBUG: contact_ptA = " << contact_ptA.transpose() << endl;
                Vector3d contact_ptB = rot_B * contact_wrt_objectB + pos_B;
                cout << "DEBUG: contact_wrt_objectB = " << contact_wrt_objectB.transpose() << endl;
                ...
                D_n_B.block<3,1>(3,0)= vec2skew(-D_n_B.block<3,1>(0,0)).transpose()*contact_wrt_objectB;
                D_n_B.block<3,1>(3,1)= vec2skew(-D_n_B.block<3,1>(0,1)).transpose()*contact_wrt_objectB;
                D_n_A.conservativeResize(6,2);
                D_n_A.block<3,1>(3,0)= vec2skew(-D_n_A.block<3,1>(0,0)).transpose()*(contact_wrt_objectA);
                D_n_A.block<3,1>(3,1)= vec2skew(-D_n_A.block<3,1>(0,1)).transpose()*(contact_wrt_objectA);
                                                        ...
                                                        double newton_tol, double linear_tol)
{
    ...
    for (int n = 0 ; n < newton_it; n++) {
        ...
        for (int colIdx = 0; colIdx < numContacts; colIdx++) {
            ...
            Vector3d contact_wrt_objectA = contacts[colIdx]->contact_wrt_objectA;
            Vector3d contact_wrt_objectB = contacts[colIdx]->contact_wrt_objectB;

            // set the contact index
            int contact_idx_bodyA = contacts[colIdx]->objectIdxA;
            int contact_idx_bodyB = contacts[colIdx]->objectIdxB;
            ...
            {
                contact_ptA = q_n.block<3, 1>(contact_idx_bodyA, 0) + contact_wrt_objectA;
            }
            else
            {
                contact_ptA =  contact_wrt_objectA;
            }
            Vector3d contact_ptB;
            if (contact_idx_bodyB != -1 )
            {
                contact_ptB = q_n.block<3,1>(contact_idx_bodyB,0) + contact_wrt_objectB;
            }
            else{
                contact_ptB =  contact_wrt_objectB;
            }
