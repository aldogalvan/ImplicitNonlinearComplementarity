...
vector<RigidObject*> rigid_objects;
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0 ; i < 3; i++)
    {
        rigid_objects.emplace_back(new RigidObject(i,"/home/aldo/ImplicitNonlinearComplementarity/resources/model.obj"));
        world->addChild(rigid_objects.back());
        if (i == 0)
        {
            rigid_objects.back()->set_local_pos(Vector3d(0.55,0.55,0.4));
            rigid_objects.back()->getMesh(0)->m_material->setBlue();
        }
        else if(i == 1)
        {
            rigid_objects.back()->set_local_pos(Vector3d(0,0.55,0.4));
            rigid_objects.back()->getMesh(0)->m_material->setRed();
        }
        else
        {
            rigid_objects.back()->set_local_pos(Vector3d(0.55,0.0,0.4));
            rigid_objects.back()->getMesh(0)->m_material->setYellow();
        }
        rigid_objects.back()->import_mesh_data();
    }

    // add a plane
    rigid_objects.emplace_back(new RigidObject(3));
    rigid_objects.back()->set_is_static(true);
    rigid_objects.back()->newMesh();
    cCreatePlane(rigid_objects.back()->getMesh(0),5,5);
    rigid_objects.back()->set_local_pos(Vector3d(0,0,0));
    rigid_objects.back()->import_mesh_data();
    rigid_objects.back()->getMesh(0)->m_material->setWhite();
    world->addChild(rigid_objects.back());
...
void updateGraphics(void)
{
    ...
    for (auto b : rigid_objects)
    {
        b->update_mesh_position();
    }
...
void updateHaptics(void) {
    ...
    ImplicitLCP* sim = new ImplicitLCP(rigid_objects);
